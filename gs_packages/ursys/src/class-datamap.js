...
DataMap.ExtractQueryData = data => {
  ...
  Object.keys(data).forEach(foundKey => {
    ...
    if (Array.isArray(value)) {
      console.warn(
        `key: ${foundKey} is an array; did you mean to use ExtractSyncData() instead?`
      );
      return;
    }
...
DataMap.ValidateCollections = cobj => {
  ...
  Object.keys(cobj).forEach(colkey => {
    ...
    if (!ok) throw Error(`${colkey}.${cmd} single value mystery error`);
...
function f_validateAdd(value, key) {
  ...
  if (subkey) {
    if (!value.id)
      throw Error(`${key}.add requires a top-level id to select subkey field`);
    if (value[subkey].id)
      throw Error(`${key}.add: ${subkey} value can not have an id`);
  } else {
...
function f_validateUpdate(value, key) {
  ...
  const vtype = typeof value;
  if (vtype !== 'object')
    throw Error(`${key}.update: requires OBJECTS with an id, not ${vtype}`);
  if (!DataMap.IsValidId(value.id))
    throw Error(`${colkey}.update has invalid id ${value.id}`);
  if (subkey) {
    if (typeof value[subkey] !== 'object')
      throw Error(`${key}.update expected sub object`);
...
function f_validateRemove(value, key) {
  ...
  if (!DataMap.IsValidId(value.id))
    throw Error(`${key}.remove has invalid id ${value.id}`);
  if (subkey) {
    if (typeof value[subkey] !== 'object')
      throw Error(`${key}.remove expects sub object with id`);
