...
UNET.NetSignal = (mesgName, data) => {
  console.warn(
    PR,
    'NOTE: Use NetPublish(), not NetSignal() since the server doesnt care.'
  );
  UNET.NetPublish(mesgName, data);
};
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
async function m_HandleMessage(socket, pkt) {
  ...
  pkt.SetData(data);
  if (DBG_NOSRV)
    console.log(PR, `'${pkt.Message()}' returning transaction data ${dbgData}`);
...
function log_PktTransaction(pkt, status, promises) {
  ...
  if (promises && promises.length) {
    console.log(
      PR,
      `${src} >> '${pkt.Message()}' ${status} ${promises.length} Promises`
    );
  } else {
