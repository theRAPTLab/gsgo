...
const TOUT = require('./util/prompts').makeTerminalOut('UNET');
...
UNET.NetCall = async (mesgName, data) => {
  ...
  if (DBG.call)
    TOUT(`${pkt.Info()} NETCALL ${pkt.Message()} to ${promises.length} remotes`);
...
UNET.NetPublish = (mesgName, data) => {
  ...
  if (DBG.call)
    TOUT(`${pkt.Info()} NETSEND ${pkt.Message()} to ${promises.length} remotes`);
};
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
async function m_HandleMessage(socket, pkt) {
  ...
  if (pkt.IsResponse()) {
    if (DBG.calls)
      TOUT(`-- ${pkt.Message()} completing transaction ${pkt.seqlog.join(':')}`);
...
function log_ListSockets(change) {
  TOUT(`socketlist changed: '${change}'`);
