/*///////////////////////////////// ABOUT \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\
  ...
  A sandbox for trying out feature development.
  This is a clone of feat-movment.
...
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
const PR = UR.PrefixUtil('FeatSandbox');
const DBG = false;
...
function moveWander(agent) {
  ...
  const distance = agent.prop.Sandbox.distance.value;
  let direction = agent.prop.Sandbox.direction.value;
  if (Math.random() > 0.98) {
    direction += Math.random() * 180;
    agent.prop.Sandbox.direction.value = direction;
  }
...
UR.SystemHook('SIM/FEATURES_UPDATE', () => {
  ...
  agents.forEach(agent => {
    // handle movement
    const type = agent.prop.Sandbox.movementType.value;
    switch (type) {
      case 'wander':
        moveWander(agent);
        break;
...
/// FEATURE CLASS /////////////////////////////////////////////////////////////
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
class SandboxPack extends Feature {
  constructor(name) {
    super(name);
...
/// REGISTER SINGLETON ////////////////////////////////////////////////////////
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
const INSTANCE = new SandboxPack('Sandbox');
Register(INSTANCE);
