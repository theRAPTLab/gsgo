...
/*///////////////////////////////// ABOUT \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*\
  ...
  lifted from https://github.com/ctrlplusb/react-tree-walker
  for INQ use, we just want to walk a tree to generate a script representation!
...
/// MAIN EXPORT ///////////////////////////////////////////////////////////////
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
/** Recurse a React Element tree, running the provided visitor against each
 *  element. If a visitor call returns `false` then we will not recurse into the
 *  respective elements children.
 *  tree: react element to walk
 *  visitor function: (element, instance, context, childContext) -> boolean
 *  context: optional object defining data context (e.g. { myItem:'foo' })
    ...
    const recursive = (currentElement, currentContext) => {
      ...
      if (currentElement.type) {
        const _context =
          ...
          (currentElement.type.Provider && currentElement.type.Provider._context);
                  ...
                  if (children) {
                    if (Array.isArray(children)) {
                      ...
                      return pMapSeries(children, child =>
                        child ? recursive(child, childContext) : Promise.resolve()
                      )
          ...
          if (
            ...
            if (isForwardRef(currentElement)) {
              ...
              const childContext = providesChildContext(instance)
                ? { ...currentContext, ...instance.getChildContext() }
                : currentContext;
