...
function SelectEditorLineSlot(props) {
  ...
  /// TEST syntax slots
  // validationTokens = {gsType, unitText, error, ...symbols}
  ...
  for (let i = 0; i < slotCount; i++) {
    ...
    if (i < syntaxTokenCount) {
      ...
      ({ label, expectedType: type, viewState } = syntax);
    } else {
    ...
    console.log('working on', i, label);
    if (i < validationTokenCount) {
      // validationToken exists, so show that
      const t = validationTokens[i];
      if (t.error) {
        // if there's an error in the token, show the current value
