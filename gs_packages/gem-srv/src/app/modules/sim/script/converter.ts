...
CMDS.set('defTemplate', {
  ...
  // this is the generator for the string
  compile: parms => {
...
function m_CompileSourceArray(source) {
  const output = {
    agent_definition: [],
    agent_defaults: [],
    agent_conditions: []
...
(window as any).linesToPrograms = () => {
  ...
  m_CompileSourceArray(source);
  return 'end test';
};
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
(window as any).tokenToCompo = () => {
